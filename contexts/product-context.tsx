...
export function ProductProvider({ children }: { children: ReactNode }) {
  ...
  const loadProduct = async (id: string | number): Promise<Product | null> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Ürün yüklenemedi'
      setError(errorMessage)
  ...
  const loadTags = async (): Promise<void> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Etiketler yüklenemedi'
      setError(errorMessage)
  ...
  const filterProducts = async (filters: Record<string, any>): Promise<void> => {
    try {
      setIsLoading(true)
      setError(null)
      ...
      const errorMessage = err instanceof Error ? err.message : 'Filtreleme yapılamadı'
      setError(errorMessage)
