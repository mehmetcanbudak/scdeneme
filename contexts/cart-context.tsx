...
import { apiClient, onApiClientReady, isApiClientReady } from '@/lib/api-client'
...
interface CartContextType {
  ...
  isLoading: boolean
  error: string | null
  addItem: (productId: number, quantity?: number, purchaseType?: 'one_time' | 'subscription', subscriptionInterval?: SubscriptionInterval, deliveryDay?: number) => Promise<{ success: boolean; message?: string }>
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // Calculate totals
  const totalItems = summary?.item_count || items.reduce((sum, item) => sum + item.quantity, 0)
  const totalPrice = summary?.subtotal || items.reduce((sum, item) => sum + item.total_price, 0)
    ...
    try {
      ...
      if (response.error) {
        ...
        if (response.error.status === 401 || 
          ...
          console.log('Cart requires authentication, user not logged in - this is expected')
          setItems([])
          setSummary(null)
          return
        }
      ...
      const data = response.data as { success: boolean, cart_items: CartItem[], summary: CartSummary | null }
  ...
  const addItem = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Ürün sepete eklenemedi'
      setError(errorMessage)
  ...
  const updateItem = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Sepet güncellenemedi'
      setError(errorMessage)
      return { success: false, message: errorMessage }
    } finally {
      setIsLoading(false)
  ...
  const removeItem = async (itemId: string): Promise<{ success: boolean; message?: string }> => {
    try {
      setIsLoading(true)
      setError(null)
      ...
      // Update local state
      setItems(prevItems => prevItems.filter(item => item.documentId !== itemId))
      
      return { success: true, message: 'Ürün sepetten çıkarıldı' }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Ürün sepetten çıkarılamadı'
      setError(errorMessage)
      return { success: false, message: errorMessage }
    } finally {
      setIsLoading(false)
  ...
  const clearCart = async (): Promise<{ success: boolean; message?: string }> => {
    try {
      ...
      const removePromises = items.map(item => apiClient.removeCartItem(item.documentId))
