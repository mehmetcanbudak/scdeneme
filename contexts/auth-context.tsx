...
interface AuthContextType {
  ...
  isLoading: boolean
  login: (phone: string, otpCode: string) => Promise<{ success: boolean; message?: string }>
  sendOTP: (phone: string) => Promise<{ success: boolean; message?: string }>
  resendOTP: (phone: string) => Promise<{ success: boolean; message?: string }>
  checkPhone: (phone: string) => Promise<{ exists: boolean; verified: boolean }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const validateTokenWithServer = async (tokenToValidate: string): Promise<{ valid: boolean; user?: User }> => {
    try {
  ...
  const refreshUserInfo = async (): Promise<{ success: boolean; message?: string }> => {
    try {
      setError(null)
  ...
  const verifyOTP = async (phone: string, otpCode: string): Promise<{ success: boolean; message?: string }> => {
    try {
      setError(null)
  ...
  const checkPhone = async (phone: string): Promise<{ exists: boolean; verified: boolean }> => {
    try {
      setError(null)
      const response = await fetch(`${API_BASE_URL}/api/auth/check-phone?phone=${encodeURIComponent(phone)}`)
