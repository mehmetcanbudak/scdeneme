...
class ApiClient {
    ...
    this.tokenPromise = new Promise((resolve) => {
      ...
      const checkToken = () => {
        if (this.isInitialized) {
          ...
          console.warn('Token initialization timeout, continuing without authentication')
          resolve(null)
        } else {
          attempts++
          // Check again in 50ms
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          if (errorMessage.includes('Authorization header missing') && !this.isAuthenticatedEndpoint(endpoint)) {
            // For public endpoints, continue without token
            console.warn(`Authorization error on public endpoint ${endpoint}, continuing...`)
          } else if (errorMessage.includes('Authorization header missing') || errorMessage.includes('invalid')) {
  ...
  async getPublicProducts(params?: {
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/products/public${queryString ? `?${queryString}` : ''}`)
  }

  async getPublicProduct(id: string | number, params?: {
    locale?: string
  }) {
    const searchParams = new URLSearchParams()
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/products/public/${id}${queryString ? `?${queryString}` : ''}`)
  }
  ...
  async getProduct(id: string | number, populate?: string) {
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/products/${id}${queryString ? `?${queryString}` : ''}`)
  }

  async getProductBySlug(slug: string, params?: {
    populate?: string
    locale?: string
  }) {
    const searchParams = new URLSearchParams()
    ...
    if (response.data && typeof response.data === 'object' && 'data' in response.data) {
      ...
      if (Array.isArray((response.data as any).data) && (response.data as any).data.length > 0) {
        return {
          ...response,
    ...
    if (response.data && typeof response.data === 'object' && 'data' in response.data && Array.isArray((response.data as any).data) && (response.data as any).data.length === 0) {
      return {
        error: {
          status: 404,
          name: 'NotFound',
  ...
  async getCategories(params?: {
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/categories${queryString ? `?${queryString}` : ''}`)
  }
  ...
  async getPublicCategories(params?: {
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/categories/public${queryString ? `?${queryString}` : ''}`)
  }
  ...
  async getSubscriptionPlans(params?: {
    ...
    const queryString = searchParams.toString()
    return this.request(`/api/subscription-plans${queryString ? `?${queryString}` : ''}`)
  }
